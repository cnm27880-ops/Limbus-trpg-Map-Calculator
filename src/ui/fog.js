...
function drawFogCanvas(t) {
    ...
    if (isSt) {
        if (fogStPreview && fogEditTargetId !== 'all') {
            drawFogPreviewAsPlayer(ctx, gridSize, t, fogEditTargetId);
        } else if (fogEditTool) {
 ...
 */
function drawFogPreviewAsPlayer(ctx, size, t, targetId) {
    ...
    for (let y = 0; y < state.mapH; y++) {
        for (let x = 0; x < state.mapW; x++) {
            ...
            fogDrawCell(ctx, x * size, y * size, size, x, y, t, temp.has(key));
        }
    }
}
