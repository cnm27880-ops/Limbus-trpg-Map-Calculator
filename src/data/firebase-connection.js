...
let _syncedMapDataStr = null;   // 上次「確認」Firebase 已同步的 mapData（JSON 字串；只在收到吻合本機內容的回音時更新）
    ...
    const mapDataListener = roomRef.child('mapData').on('value', snapshot => {
        if (snapshot.exists()) {
            ...
            if (typeof validateMapData === 'function') {
                validated = validateMapData(raw, 50, 50);
                if (!validated) {
            ...
            const incomingStr = JSON.stringify(validated);
            const localStr = JSON.stringify(state.mapData);
            // 「本機有尚未確認同步的編輯」＝已收過首個快照、且本機內容跟上次確認同步的內容不同
            // （debounce 期間、或已送出寫入但回音尚未抵達、抵達前又有新編輯，都算在內）。
            const localDirty = _mapDataReceivedOnce && (localStr !== _syncedMapDataStr);

            // 本機有未同步編輯、且這筆快照內容跟本機目前內容不同：代表這多半是「較早一次寫入」
            // 的回音，在本機又編輯之後才抵達；若直接套用會用舊內容蓋掉本機還沒送出的新編輯
            // （syncMapData 的 debounce 會把「尚未送出的新編輯」的網路寫入延後，但這裡的
            // 監聽器完全獨立、隨時可能收到「更早那次寫入」的回音，兩者之間才是真正的競態）。
            // 略過即可：真正對應「本機目前內容」的回音，或稍後 debounce 補送，最終仍會一致。
            if (localDirty && incomingStr !== localStr) {
                return;
            }

            state.mapData = validated;
            _syncedMapDataStr = incomingStr;
            _mapDataReceivedOnce = true;
            scheduleRenderMap();
        }
    });
...
function syncMapData() {
    ...
    // 注意：不在此處樂觀標記 _syncedMapDataStr。若在回音抵達前使用者又畫了新的一筆，
    // 提早標記會讓上方監聽器誤以為「內容已同步」，之後續就算內容又变了也不會再送出去
    // （debounce 到期時發現跟 _syncedMapDataStr 不同才會補送，提早標記會蓋掉這個判斷依據）。
    // 真正的標記交給 mapData 監聽器，收到「內容吻合本機目前資料」的回音時才確認。
}
